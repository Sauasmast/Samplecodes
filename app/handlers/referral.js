...
module.exports.getdata = async (req, res) => {
	...
	try {
		...
		const { type, max_referral_count, default_count } = await referral.userConfigdata(req.request_id, user_id);
		const { active, users_referred } = await referral.userreferreddata(req.request_id, user_id);
		const response_body = {
			type,
			max_referral_count,
			default_count,
